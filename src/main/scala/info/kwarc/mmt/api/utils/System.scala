...
object OSystem {
  ...
  def getLatestVersion: (String, String) = {
    val json = JSONFromURL("https://api.github.com/repos/Uniformal/MMT/releases/latest").getOrElse(throw new Error("Unable to load JSON")).asInstanceOf[JSONObject]
  ...
  lazy val legalNotices: String = OSystem.getResourceList("/legal/").sorted.collect({
      case s: String if s.endsWith(".txt") => OSystem.getResourceAsString("/legal/" + s)
    }).mkString("\n")
  ...
  val userConfigFile = {
   ...
   def getResourceAsString(path: String): String = Option(getResource(path)) match {
     ...
     case None => throw new Error(s"cannot find resource $path (run style is $runStyle)")
   }
