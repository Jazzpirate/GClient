...
case class JSONObject(map: List[(JSONString, JSON)]) extends JSON {
  ...
  def getAs[A](cls: Class[A], s : String) : A = {
    val ret = apply(s).getOrElse(throw new Error("Field \"" + s + "\" not defined in JSONObject " + this)) match {
      case j : JSONValue => j.value
      case other => other
    }
    ret match {
      case j: A@unchecked if cls.isInstance(j) => j
      case _ => throw new Error("getAs Error: A=" + A.toString + ", j:" + ret.getClass)
    }
  }
  ...
  def getAsInt(s : String) : BigInt = apply(s).getOrElse(throw new Error("Field \"" + s + "\" not defined in JSONObject " + this)) match {
    case j: JSONInt => j.value
    case _ => throw new Error("Field \"" + s + "\" is not a JSONInt in " + this)
  }
  def getAsFloat(s : String) : BigDecimal = apply(s).getOrElse(throw new Error("Field \"" + s + "\" not defined in JSONObject " + this)) match {
  ...
  def getAsList[A](cls: Class[A],s : String) : List[A] = {
    val ret = apply(s).getOrElse(throw new Error("Field \"" + s + "\" not defined in JSONObject " + this)) match {
      ...
      case _ => throw new Error("Field \"" + s + "\" not da JSONArray " + this)
    }
    ret map {
      ...
      case j : A@unchecked if cls.isInstance(j) => j
      case j => throw new Error("getAs Error: A=" + cls.toString + ", j:" + j.getClass + " = " + j)
    }
  }
}
...
object JSON {
  ...
  def quoteString(s : String): String = s.map {
     ...
     */
    case c : Char if (c >= '\u0000' && c <= '\u001f') || (c >= '\u007f' && c <= '\u009f') => "\\u%04x".format(c.toInt)
    case c => c
  }.mkString
}
...
object JSONFromURL {
  def apply(url: String) : Option[JSON] = {
    ...
    Try(scala.io.Source.fromURL(url)).toOption.map { a =>
      Try(JSON.parse(a.toBuffer.mkString)) match {
        case util.Failure(t: Throwable) =>
          throw new Error(url.toString)//.setCausedBy(t)
