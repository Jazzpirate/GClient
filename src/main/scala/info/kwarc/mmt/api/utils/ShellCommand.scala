...
object ShellCommand {
  ...
  case class Abort(e: Exception) extends Result(Some(new Error("shell command aborted")))
  case class Fail(output: String, exitValue: Int) extends Result(Some(new Error("unknown error: " + output)))
  case class Success(output: String) extends Result(None)

  private def runInGeneral(dir: Option[utils.File], env: Map[String,String], command: String*)(out: String => Unit): Result = {
    // running shell commands is always brittle; better always print what we're trying to do 
    out("running command: " + command.mkString(" "))
    out("in directory: " + dir.getOrElse(utils.File(System.getProperty("user.dir"))).toString)
   ...
   */
  def runAndPrint(dir: Option[utils.File], command: String*) = {
